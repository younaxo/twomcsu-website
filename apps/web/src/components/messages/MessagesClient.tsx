  ...
  return (
    <Avatar className={dimensions}>
      ...
      <AvatarFallback className="p-0">
        <span className="flex h-full w-full items-center justify-center bg-primary/15 text-primary">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-2">
        {items.length ? (
                  ...
                  <p className="min-w-0 flex-1 truncate text-xs text-muted-foreground">
                    {conversation.lastMessage?.isDeleted
                      ...
                      : conversation.lastMessage?.content ||
                        (conversation.lastMessage?.attachments.length ? 'Файл' : 'Нет сообщений')}
                  </p>
          ...
          <EmptyState
            ...
            description="Начните новый разговор с другом или участником сообщества."
          />
        )}
      </div>
    </aside>
...
function MessageAttachmentView({
  ...
  attachment: DirectMessage['attachments'][number];
}) {
  ...
  return (
    ...
    >
      ...
      <span className="text-[10px] text-muted-foreground">
        {Math.ceil(attachment.size / 1024)} КБ
      </span>
    </a>
  );
}
  ...
  return (
    <div className={cn('group flex items-end gap-2', own ? 'justify-end' : 'justify-start')}>
      ...
      <div className={cn('max-w-[88%] sm:max-w-[75%]', own ? 'items-end' : 'items-start')}>
        {!own ? (
          <p className="mb-1 px-2 text-xs font-medium text-primary">
            {message.sender?.username ?? 'Удалённый пользователь'}
          </p>
        ) : null}
        <div
          className={cn(
          ...
          {message.parent ? (
            <div className="mb-2 border-l-2 border-primary/60 pl-2 text-xs text-muted-foreground">
              <p className="font-medium text-foreground/80">
                {message.parent.sender?.username ?? 'Пользователь'}
              </p>
...
export function MessagesClient() {
  ...
  const filtered = useMemo(() => {
    ...
    return (conversations.data?.items ?? []).filter(
  ...
  const upsertMessage = useCallback(
    (message: DirectMessage) => {
      qc.setQueryData<DirectMessagesResponse>(
        queryKeys.directMessages(message.conversationId),
        (current) => {
  ...
  useEffect(() => {
    ...
    const onTyping = (payload: { conversationId: string; userId: string; username: string }) => {
      if (payload.conversationId === activeId)
        setTypingUsers((current) => ({ ...current, [payload.userId]: payload.username }));
    };
  ...
  const activeConversation =
    details.data ?? conversations.data?.items.find((item) => item.id === activeId);
      ...
      {activeId && activeConversation ? (
          ...
          <header className="flex h-16 shrink-0 items-center gap-3 border-b border-white/5 px-3 sm:px-4">
            ...
            {activeConversation.type === ConversationType.GROUP ? (
              ...
              <Button variant="ghost" size="sm" asChild>
                <Link href={`/users/${activeConversation.title}`}>Профиль</Link>
              </Button>
            )}
          </header>
          ...
          <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-3 py-4 sm:px-5">
            {messages.isLoading
              ? Array.from({ length: 6 }).map((_, index) => (
                  <Skeleton
                    key={index}
                    className={cn('h-16 w-2/3 rounded-2xl', index % 2 && 'ml-auto')}
                  />
                ))
              : null}
            {!messages.isLoading && !messages.data?.items.length ? (
              <EmptyState
                ...
                description="Сообщения поддерживают Markdown, ответы, реакции и файлы."
              />
            ) : null}
            {messages.data?.items.map((message) => {
              ...
              const read =
                own &&
                activeConversation.members.some(
                  (member) =>
              ...
              return (
                <MessageBubble
                  ...
                  onDelete={async () => {
                    try {
                      upsertMessage(await remove.mutateAsync(message.id));
                    } catch (error) {
                      toast.error(extractErrorMessage(error, 'Не удалось удалить сообщение'));
                    }
                  }}
                  onReact={async (emoji) => {
                    try {
                      upsertMessage(await react.mutateAsync({ messageId: message.id, emoji }));
                    } catch (error) {
                      toast.error(extractErrorMessage(error, 'Не удалось поставить реакцию'));
                    }
                  }}
                />
              );
              ...
              <div className="mb-2 flex items-center gap-2 rounded-xl border-l-2 border-primary bg-white/5 px-3 py-2 text-xs">
                <div className="min-w-0 flex-1">
                  <p className="font-medium text-primary">
                    {editing
                      ? 'Редактирование'
                      : `Ответ: ${reply?.sender?.username ?? 'пользователь'}`}
                  </p>
                  <p className="truncate text-muted-foreground">
                    {editing?.content ?? reply?.content}
                  </p>
                </div>
                <Button
                  size="icon"
        ...
        <section className="hidden h-full items-center justify-center lg:flex">
          <EmptyState
            ...
            description="Или начните новый разговор с пользователем."
          />
        </section>
      )}
    </div>
