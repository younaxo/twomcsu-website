...
export default function AdminDepartmentsPage() {
  ...
  const loadUserDepartments = useCallback(async (username: string) => {
    try {
      const { data } = await api.get<UserProfile>(`/users/${encodeURIComponent(username)}/public`, {
  ...
  if (!isAdmin) {
    return (
      <p className="text-sm text-muted-foreground">Управление отделами доступно администраторам.</p>
    );
  }
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            >
              <SelectTrigger>
                <SelectValue
                  placeholder={atMax ? `Максимум ${MAX_USER_DEPARTMENTS} отдела` : 'Выберите отдел'}
                />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="flex flex-wrap gap-2">
            <Button onClick={assign} disabled={!selectedUser || !assignId || atMax}>
