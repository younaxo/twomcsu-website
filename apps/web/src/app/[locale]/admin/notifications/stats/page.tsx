...
export default function AdminNotificationStatsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-foreground">Статистика уведомлений</h1>
      ...
      <section className="rounded-2xl border border-border bg-card p-5">
        <h2 className="mb-3 text-lg font-medium text-foreground">По типам</h2>
        <ul className="space-y-2 text-sm">
          {Object.entries(data.byType).map(([type, count]) => (
            <li key={type} className="flex justify-between gap-3 border-b border-border py-2">
