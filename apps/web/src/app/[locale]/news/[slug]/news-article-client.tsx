...
export function NewsArticleClient({ slug }: NewsArticleClientProps) {
  ...
  const toc = useMemo(() => (detail.data ? extractToc(detail.data.content) : []), [detail.data]);
  ...
  const shareUrl =
    typeof window !== 'undefined' ? window.location.href : `https://twomc.su/news/${news.slug}`;
  const relatedItems = related.data?.data.filter((item) => item.id !== news.id).slice(0, 3) ?? [];
