...
function PodiumCard({ entry, metric }: { entry: LeaderboardEntry; metric: LeaderboardMetric }) {
  return (
    <Link href={`/users/${entry.user.username}`} className={cn(
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-8 px-4 py-8 sm:px-6">
      <header className="relative overflow-hidden rounded-3xl border border-white/10 glass-strong p-6 sm:p-9">
        ...
        <h1 className="font-display relative mt-4 text-3xl text-white sm:text-5xl">Таблицы лидеров</h1>
        <p className="relative mt-3 max-w-2xl text-muted-foreground">
          Рейтинг формируется по игровой статистике. Игроки со скрытой статистикой в него не попадают.
        </p>
      </header>
      ...
      {leaderboard.isLoading ? (
        <div className="grid gap-4 md:grid-cols-3">{Array.from({ length: 3 }).map((_, index) => <Skeleton key={index} className="h-64 rounded-3xl" />)}</div>
      ) : null}

      {!leaderboard.isLoading && !leaderboard.data?.items.length ? (
        <EmptyState icon={Medal} title="Рейтинг пока пуст" description="Данные появятся после синхронизации статистики с игровым сервером." />
      ) : null}

      {podium.length ? (
        <section className="grid items-end gap-4 pt-4 md:grid-cols-3">
          {podium.map((entry) => <PodiumCard key={entry.user.id} entry={entry} metric={metric} />)}
        </section>
      ) : null}
        ...
        <Card className="overflow-hidden border-white/10 glass-medium">
          <CardContent className="divide-y divide-white/5 p-0">
            {leaderboard.data?.items.slice(3).map((entry) => (
              <Link key={entry.user.id} href={`/users/${entry.user.username}`} className="grid grid-cols-[2rem_1fr_auto] items-center gap-3 px-4 py-3 transition-colors hover:bg-white/[0.04] sm:grid-cols-[3rem_1fr_8rem_8rem_auto]">
                <span className="font-display text-center text-lg text-muted-foreground">#{entry.rank}</span>
                <div className="flex min-w-0 items-center gap-3">
                  <AvatarWithSkin user={entry.user} size="sm" />
                  <span className="truncate font-medium text-white">{entry.user.username}</span>
                </div>
                <span className="hidden items-center gap-1.5 text-sm text-muted-foreground sm:flex"><Swords className="h-4 w-4" />{entry.kills}</span>
                <span className="hidden items-center gap-1.5 text-sm text-muted-foreground sm:flex"><Shield className="h-4 w-4" />{entry.deaths}</span>
                <span className="flex items-center gap-2 font-semibold text-primary"><Clock3 className="hidden h-4 w-4 sm:block" />{formatValue(metric, entry.value)}</span>
              </Link>
            ))}
          </CardContent>
        </Card>
      ...
      {leaderboard.data ? (
        <p className="text-center text-xs text-muted-foreground">Обновлено: {new Date(leaderboard.data.updatedAt).toLocaleString('ru-RU')}</p>
      ) : null}
    </div>
  );
}
