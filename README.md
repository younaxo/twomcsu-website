...
Акцент оранжевый (`#F57C00` / `#FF9800`), логотип без glow/scale, sticky glassmorphism header + левая sidebar (свёрнутая ~88px / раскрытая 240px; Магазин/Серверы/Новости/Вики/Обращения/Репорты, снизу Чат и Корзина; для HELPER+/ADMIN+ — Модерация/Админ/Дашборд), утилиты `.glass-panel` / `.glass-card` / `.glass-modal`, валюта в хедере пересчитывает цены на всём сайте. Уведомления: glass-карточки, свайп влево — прочитать, вправо — удалить.
