...
Визуальная система: спокойный тёмный Apple-like интерфейс с оранжевым акцентом
(`#F57C00` / `#E65100`), чистой иерархией и умеренным Liquid Glass. Основной шрифт — Onest,
заголовки — Dela Gothic One, логотип и компактные акценты — Geologica.
Плавающий glass header (уведомления + профиль с бейджем и chevron) и компактная левая
навигационная панель (категории Основное/Сообщество, снизу Чат, Корзина и выбор валюты).
...
Публичная страница `/streams` объединяет прямые эфиры Twitch и YouTube. Партнёрские
каналы отображаются при любом эфире; для остальных каналов название эфира должно
содержать `twomc.su`. Проверка выполняется автоматически раз в пять минут и вручную
из `/admin/streams`.

| Метод и путь                           | Доступ    | Что делает                        |
| -------------------------------------- | --------- | --------------------------------- |
| `GET /streams`                         | публичный | Активные каналы и состояние эфира |
| `GET/POST/PATCH/DELETE /admin/streams` | ADMIN+    | Управление каналами               |
| `POST /admin/streams/refresh`          | ADMIN+    | Немедленно проверить эфиры        |

Для проверки Twitch задаются `TWITCH_CLIENT_ID` и `TWITCH_CLIENT_SECRET`, для
YouTube — `YOUTUBE_API_KEY`. Без ключа соответствующая платформа остаётся доступной
в каталоге, а администратор видит сообщение о ненастроенной интеграции.

## Голосование (этап 22)

Страница `/vote` показывает сайты мониторинга, награду в рубинах и персональный
таймер до следующего голоса. Сайты и webhook-секреты настраиваются в
`/admin/voting`; секрет хранится в базе только в виде SHA-256 хеша и показывается
администратору один раз.

| Метод и путь                                 | Доступ                    | Что делает                             |
| -------------------------------------------- | ------------------------- | -------------------------------------- |
| `GET /voting`                                | публичный (+optional JWT) | Сайты, награды и cooldown пользователя |
| `POST /voting/webhook/:slug`                 | webhook + `x-vote-secret` | Подтвердить голос и начислить рубины   |
| `GET/POST/PATCH/DELETE /admin/voting/sites`  | ADMIN+                    | Управление сайтами                     |
| `POST /admin/voting/sites/:id/rotate-secret` | ADMIN+                    | Заменить webhook-секрет                |

Тело webhook: `username` и необязательный уникальный `externalId`. Повторная
доставка одного `externalId` идемпотентна; начисление награды и запись голоса
выполняются в транзакции.

## Награды, мини-игры и рефералы (этапы 23–25)

- `/rewards` — выбор одной из восьми ежедневных карточек, семидневная серия с
  легендарными призами и еженедельное колесо редкости.
- `/minigames` — рулетка, краш и апгрейдер только за виртуальные рубины. Ставки и
  выплаты выполняются транзакционно, результаты содержат seed и SHA-256 хеш.
- `/referrals` — персональные ссылки, три уровня приглашений, награды 100/40/20
  рубинов и публичный лидерборд.
- `/media` — кабинет партнёров Twitch, TikTok и YouTube с четырьмя рангами,
  промокодом, статистикой регистраций и генератором PNG-баннеров.
- `/admin/media-partners` и `/admin/media-requests` — управление рангами и
  одобрение заявок.

Ближайшие этапы: Multilanguage (RU/EN/UA), Cookie Consent + Privacy и API для разработчиков.
Уровни игроков запланированы после Minecraft Sync.
